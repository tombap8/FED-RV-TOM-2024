...
function saveSearchHistory(txt) {
  ...
  let memory = localStorage.getItem("memory-data") || "";
...
function displaySearchHistory() {
  ...
  if (memory && memory !== "") {
    ...
    memArr.reverse().forEach(function (v) {
      $historyList.append(
        `<li><b onclick="reSearch('${v}')">${v}</b></li>`
      );
    });
  } else {
...
function goFirstPaging() {
  pgPgNum = 1;
  pageNum = 1;
  renderMode();
}

function goPrevPaging(initNum) {
  pgPgNum--;
  pageNum = initNum - (pgPgSize - 1);
  renderMode();
}

function goNextPaging(limitNum) {
  pgPgNum++;
  pageNum = limitNum + 1;
  renderMode();
}

function goLastPaging(pgPgLimit) {
  pgPgNum = pgPgLimit;
...
function increaseViewCount(idx) {
  ...
  // 로그인 사용자의 글이면 증가 안함
  if (loginSts && selRecord.uid === loginSts.uid) {
    isRec = true;
  }
...
function renderReadMode() {
  ...
  $("#read-name").val(selRecord.unm);
  $("#read-title").val(selRecord.tit);
  $("#read-content").val(selRecord.cont);

  // 수정 버튼 표시 (로그인 + 작성자)
  if (loginSts && loginSts.uid === selRecord.uid) {
    $("#readModifyBtn").show();
  } else {
    $("#readModifyBtn").hide();
  }
...
function saveComment() {
  ...
  let newIdx =
    comDt.length > 0 ? Math.max(...comDt.map((v) => v.idx)) + 1 : 1;
...
function displayComments() {
  ...
  if (comDt.length > 0) {
    ...
    comDt.forEach((v, i) => {
      ...
      if (loginSts && loginSts.uid === v.uid) {
      ...
      html += `</td>
        <td>
          <textarea class="comment-view-box" id="comment-${v.idx}" 
            ${isEditing ? '' : 'readonly'}
            ${isEditing ? 'class="comment-view-box editing"' : 'class="comment-view-box"'}
          >${v.cont}</textarea>
        </td>
...
function saveModifiedComment(idx) {
  let newContent = $(`#comment-${idx}`).val();
  let comDt = JSON.parse(localStorage.getItem("comment-data"));

  comDt = comDt.map((v) =>
    v.idx === idx ? { ...v, cont: newContent } : v
  );
